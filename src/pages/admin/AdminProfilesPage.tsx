...
import {
  ...
  ArrowDown, Check, X, Filter, Briefcase, User as ProfileIcon, AlertTriangle,
} from 'lucide-react';
import {
...
const mockProfiles: ProfileData[] = [
  ...
  { id: '3', email: 'maria.lopez@example.com', profileHandle: 'marialopez', nombre_completo: 'Maria Lopez', avatar_url: 'https://i.pravatar.cc/150?u=maria', rol: 'Admin', status: 'active', fecha_registro: '2023-03-20T12:00:00Z', ultimo_login: '2024-01-14T18:45:00Z', ubicacion: 'Valencia, Spain', portfolio_views: 2345, proyectos_count: 6, conexiones_count: 89 },
  { id: '4', email: 'pedro.sanchez@example.com', profileHandle: 'pedros', nombre_completo: 'Pedro Sanchez', avatar_url: 'https://i.pravatar.cc/150?u=pedro', rol: 'Estandar', status: 'pending', fecha_registro: '2024-01-10T15:30:00Z', portfolio_views: 12, proyectos_count: 0, conexiones_count: 0 },
  { id: '5', email: 'laura.garcia@example.com', profileHandle: 'lauragarcia', nombre_completo: 'Laura Garcia', avatar_url: 'https://i.pravatar.cc/150?u=laura', rol: 'Estandar', status: 'banned', fecha_registro: '2023-08-05T09:20:00Z', ultimo_login: '2023-12-20T11:30:00Z', ubicacion: 'Sevilla, Spain', portfolio_views: 567, proyectos_count: 3, conexiones_count: 45 },
  { id: '6', email: 'javier.fernandez@example.com', profileHandle: 'javierfernandez', nombre_completo: 'Javier Fernandez', avatar_url: 'https://i.pravatar.cc/150?u=javier', rol: 'Reclutador', status: 'active', fecha_registro: '2023-05-12T11:00:00Z', ultimo_login: '2024-01-16T10:30:00Z', ubicacion: 'Bilbao, Spain', empresa: 'HR Solutions', portfolio_views: 890, proyectos_count: 2, conexiones_count: 178 },
  { id: '7', email: 'sofia.torres@example.com', profileHandle: 'sofiatorres', nombre_completo: 'Sofia Torres', avatar_url: 'https://i.pravatar.cc/150?u=sofia', rol: 'Estandar', status: 'active', fecha_registro: '2023-09-01T14:15:00Z', ultimo_login: '2024-01-15T16:00:00Z', ubicacion: 'Malaga, Spain', portfolio_views: 1456, proyectos_count: 12, conexiones_count: 234 },
  { id: '8', email: 'diego.navarro@example.com', profileHandle: 'diegonavarro', nombre_completo: 'Diego Navarro', avatar_url: 'https://i.pravatar.cc/150?u=diego', rol: 'Estandar', status: 'active', fecha_registro: '2023-11-20T09:45:00Z', ultimo_login: '2024-01-14T12:00:00Z', ubicacion: 'Zaragoza, Spain', empresa: 'DevStudio', portfolio_views: 678, proyectos_count: 5, conexiones_count: 67 },
];

const roleOptions = [
  { value: 'all', label: 'Todos los Roles', icon: Users },
  { value: 'Estandar', label: 'Estandar', icon: ProfileIcon },
...
export default function AdminProfilesPage() {
  ...
  const [confirmAction, setConfirmAction] = useState<{ type: string; profileId: string } | null>(null);
  ...
  const filteredProfiles = useMemo(() => {
    return profiles
      .filter((profile) => {
        ...
        const matchesSearch =
          profile.nombre_completo.toLowerCase().includes(searchLower) ||
          profile.email.toLowerCase().includes(searchLower) ||
          profile.profileHandle.toLowerCase().includes(searchLower);
        const matchesStatus = statusFilter === 'all' || profile.status === statusFilter;
        const matchesRole = roleFilter === 'all' || profile.rol === roleFilter;
  ...
  }, [profiles, searchQuery, statusFilter, roleFilter, sortBy, sortOrder]);
  ...
  const handleSelectAll = () => {
    if (selectedProfiles.size === paginatedProfiles.length) {
      setSelectedProfiles(new Set());
    } else {
      setSelectedProfiles(new Set(paginatedProfiles.map(u => u.id)));
    }
  };
  ...
  const handleAction = (action: string, profile: ProfileData) => {
    if (action === 'view') {
      ...
      setConfirmAction({ type: action, profileId: profile.id });
  ...
  const confirmActionHandler = () => {
    if (confirmAction) {
      setProfiles((prev) =>
        prev.map((u) => {
          if (u.id === confirmAction.profileId) {
  ...
  const handleChangeRole = (newRole: 'Estandar' | 'Reclutador' | 'Admin') => {
    if (selectedProfile) {
      setProfiles((prev) => prev.map((u) => (u.id === selectedProfile.id ? { ...u, rol: newRole } : u)));
  ...
  const handleBulkExport = () => {
    const selectedData = profiles.filter((u) => selectedProfiles.has(u.id));
    const csv = [
      ['Email', 'Nombre', 'ProfileHandle', 'Rol', 'Estado', 'Fecha Registro'].join(','),
      ...selectedData.map((u) => [u.email, u.nombre_completo, u.profileHandle, u.rol, u.status, u.fecha_registro].join(',')),
    ].join('\n');
  ...
  const getStatusBadge = (status: ProfileData['status']) => {
    const variants = {
  ...
  const getRoleBadge = (rol: ProfileData['rol']) => {
    const variants = {
    ...
    <div className="max-w-full space-y-4 overflow-x-hidden bg-gray-50 dark:bg-black p-4 md:space-y-6 md:p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400 md:text-base">{profiles.length.toLocaleString()} usuarios registrados</p>
        </div>
        <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:gap-3">
          {selectedProfiles.size > 0 && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex items-center gap-2">
              <span className="text-xs text-violet-600 dark:text-violet-300 sm:text-sm">{selectedProfiles.size} seleccionados</span>
      ...
      <div className="space-y-3 md:space-y-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Buscar por email, nombre o profileHandle..."
            value={searchQuery}
            ...
            <div className="block md:hidden">
              <div className="flex items-center justify-between border-b border-gray-200 dark:border-violet-500/20 bg-gray-50 dark:bg-violet-500/5 px-4 py-3">
                <button onClick={handleSelectAll} className={cn("flex items-center gap-2 text-xs font-medium", selectedProfiles.size === paginatedProfiles.length ? "text-violet-600 dark:text-violet-400" : "text-gray-500 dark:text-violet-300/70")}>
                  <span className={cn("flex h-4 w-4 items-center justify-center rounded border transition-all", selectedProfiles.size === paginatedProfiles.length ? "border-violet-500 bg-violet-500 text-white" : "border-gray-300 dark:border-violet-500/30")}>
                    {selectedProfiles.size === paginatedProfiles.length && <Check className="h-2.5 w-2.5" />}
                  </span>
                  Seleccionar todos
                </button>
              </div>
              <div className="divide-y divide-gray-100 dark:divide-violet-500/10">
                <AnimatePresence mode="popLayout">
                  {paginatedProfiles.map((profile, index) => (
                    <motion.div key={profile.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ delay: index * 0.03 }} className={cn("flex flex-col gap-3 p-4 transition-colors", selectedProfiles.has(profile.id) && "bg-violet-50 dark:bg-violet-500/10")}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                          <button onClick={() => handleSelectProfile(profile.id)} className={cn("flex h-5 w-5 flex-shrink-0 items-center justify-center rounded border transition-all", selectedProfiles.has(profile.id) ? "border-violet-500 bg-violet-500 text-white" : "border-gray-300 dark:border-violet-500/30")}>
                            {selectedProfiles.has(profile.id) && <Check className="h-3 w-3" />}
                          </button>
                          <Avatar src={profile.avatar_url} name={profile.nombre_completo} size="md" className="flex-shrink-0 border border-gray-200 dark:border-violet-500/20" />
                          <div className="min-w-0">
                            <p className="truncate font-medium text-black dark:text-white">{profile.nombre_completo}</p>
                        ...
                        <div className="group relative flex-shrink-0">
                          ...
                          <div className="invisible absolute right-0 top-full z-50 mt-2 min-w-[180px] rounded-xl border border-gray-200 dark:border-violet-500/30 bg-white dark:bg-black/95 p-2 opacity-0 shadow-xl transition-all group-focus-within:visible group-focus-within:opacity-100 group-hover:visible group-hover:opacity-100">
                            <button onClick={() => handleAction('view', profile)} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-black dark:text-violet-300 hover:bg-gray-50 dark:hover:bg-violet-500/10"><Eye className="h-4 w-4" /> Ver Perfil</button>
                            <button onClick={() => handleAction('changeRole', profile)} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-black dark:text-violet-300 hover:bg-gray-50 dark:hover:bg-violet-500/10"><UserCog className="h-4 w-4" /> Cambiar Rol</button>
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-wrap items-center gap-2 pl-8">
                        {getRoleBadge(profile.rol)} {getStatusBadge(profile.status)}
                        <span className="inline-flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400"><Calendar className="h-3 w-3" /> {formatDate(profile.fecha_registro)}</span>
                      </div>
                    </motion.div>
                  ))}
                </AnimatePresence>
              ...
              <table className="w-full min-w-[800px]">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-violet-500/20 bg-gray-50 dark:bg-violet-500/5">
                    <th className="w-12 px-4 py-4">
                      <button onClick={handleSelectAll} className={cn("flex h-5 w-5 items-center justify-center rounded border transition-all", selectedProfiles.size === paginatedProfiles.length ? "border-violet-500 bg-violet-500 text-white" : "border-gray-300 dark:border-violet-500/30")}>
                        {selectedProfiles.size === paginatedProfiles.length && <Check className="h-3 w-3" />}
                      </button>
                    </th>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-violet-500/10">
                  {paginatedProfiles.map((profile) => (
                    <tr key={profile.id} className="hover:bg-gray-50 dark:hover:bg-violet-500/5">
                      <td className="px-4 py-4">
                        <button onClick={() => handleSelectProfile(profile.id)} className={cn("flex h-5 w-5 items-center justify-center rounded border transition-all", selectedProfiles.has(profile.id) ? "border-violet-500 bg-violet-500 text-white" : "border-gray-300 dark:border-violet-500/30")}>
                          {selectedProfiles.has(profile.id) && <Check className="h-3 w-3" />}
                        </button>
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-3">
                          <Avatar src={profile.avatar_url} name={profile.nombre_completo} size="md" className="border border-gray-200 dark:border-violet-500/20" />
                          <div><p className="font-medium text-black dark:text-white">{profile.nombre_completo}</p><p className="text-sm text-gray-500 dark:text-gray-400">{profile.email}</p></div>
                        </div>
                      </td>
                      <td className="px-4 py-4">{getRoleBadge(profile.rol)}</td>
                      <td className="px-4 py-4">{getStatusBadge(profile.status)}</td>
        ...
        {selectedProfile && (
          <div className="space-y-6 text-black dark:text-white">
            <div className="flex items-center gap-4">
              <Avatar src={selectedProfile.avatar_url} name={selectedProfile.nombre_completo} size="xl" className="border-2 border-violet-500/30" />
              <div><h3 className="text-xl font-semibold">{selectedProfile.nombre_completo}</h3><p className="text-gray-500 dark:text-gray-400">@{selectedProfile.profileHandle}</p></div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><p className="text-xs text-gray-500">Email</p><p>{selectedProfile.email}</p></div>
              <div><p className="text-xs text-gray-500">Registro</p><p>{formatDate(selectedProfile.fecha_registro)}</p></div>
            </div>

...
function AttachmentTypeIcon({ type }: { type: AttachmentCategory }) {
  switch (type) {
    case 'image': return <Image className="h-4 w-4 text-violet-400 shrink-0" />;
    case 'audio': return <Music className="h-4 w-4 text-violet-400 shrink-0" />;
    case 'video': return <Video className="h-4 w-4 text-violet-400 shrink-0" />;
    default:      return <FileText className="h-4 w-4 text-violet-400 shrink-0" />;
  }
}
  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        isActive
          ? 'bg-gradient-to-r from-violet-500/20 to-violet-500/8 border border-violet-500/25 shadow-md shadow-violet-500/10'
    ...
    >
      <div className="relative shrink-0">
        <div className={cn(
          'rounded-full p-0.5',
          isActive ? 'bg-gradient-to-br from-violet-400 to-violet-600' : 'bg-transparent',
        )}>
          <Avatar
        ...
        {chat.unread > 0 && (
          <motion.span
            ...
            className="absolute -right-1 -top-1 flex h-5 min-w-[20px] items-center justify-center rounded-full bg-violet-500 px-1 text-[10px] font-bold text-white ring-2 ring-card shadow-lg shadow-violet-500/40"
          >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-1">
          ...
          {chat.last_message_at && (
            <span className={cn(
              'shrink-0 text-[10px]',
              chat.unread > 0 ? 'text-violet-400 font-semibold' : 'text-muted-foreground',
    ...
    >
      ...
      >
        ...
        <p className="mt-2 text-sm text-muted-foreground leading-relaxed">
          Solo se eliminará de tu vista. El otro participante conservará sus mensajes.
        </p>
      ...
      >
        <div className="relative">
          <motion.div
            ...
            className="flex h-24 w-24 items-center justify-center rounded-3xl bg-gradient-to-br from-violet-500/20 to-violet-600/10 shadow-xl shadow-violet-500/10 border border-violet-500/20"
          >
...
export default function ProfessionalChatPage() {
  ...
  const loadChats = useCallback(async () => {
    ...
    try {
      const { data, error } = await sb.rpc('get_professional_chat_list', { p_basic_id: profile.id });
      ...
      const enriched: ChatContact[] = await Promise.all(
        rows.map(async (chat) => {
          ...
          const name = chat.company_name || [chat.first_name, chat.last_name].filter(Boolean).join(' ') || 'Empresa';
          return {
            chat_id: chat.chat_id,
            other_profile_id: chat.company_profile_id,
            other_name: name,
  ...
  useEffect(() => {
    ...
    const msgChannel = sb.channel(`pro-chat:${activeChatId}`)
    ...
    const typingChannel = sb.channel(`typing:${activeChatId}`)
  ...
  useEffect(() => {
    if (activeChatId) navigate(`/dashboard/chat/${activeChatId}`, { replace: true });
    else navigate('/dashboard/chat', { replace: true });
  ...
  const handleDeleteChat = useCallback(async () => {
    if (!activeChatId || !profile?.id || !supabase) return;
    try {
      const { error } = await supabase.rpc('delete_chat', { p_chat_id: activeChatId, p_basic_id: profile.id });
      if (error) throw error;
          ...
          <div className="flex h-16 shrink-0 items-center gap-3 border-b border-border/60 bg-gradient-to-r from-violet-500/8 to-transparent px-4">
            ...
            {chats.length > 0 && (
              <motion.span
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className="ml-auto flex h-5 min-w-[20px] items-center justify-center rounded-full bg-violet-500/15 px-1.5 text-[10px] font-semibold text-violet-400 ring-1 ring-violet-500/20"
              >
                {chats.length}
              </motion.span>
            )}
            ...
            {loadingChats ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center px-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground/60 leading-relaxed">
                  {searchQuery ? 'Prueba con otro nombre' : 'Los reclutadores que te contacten aparecerán aquí'}
                </p>
              </div>
            ) : (
              <AnimatePresence>
            ...
            <>
              ...
              <div className="relative flex h-16 shrink-0 items-center gap-3 border-b border-border/60 bg-card/80 backdrop-blur-sm px-4">
                ...
                {activeContact && (
                  ...
                  >
                    <div className="relative shrink-0">
                      <div className="rounded-full p-0.5 bg-gradient-to-br from-violet-400/50 to-violet-600/30">
                        <Avatar
                  ...
                  <AnimatePresence>
                    {attachmentFile && (
                      <motion.div
                        ...
                        className="border-t border-border/60 bg-violet-500/5"
                      >
                        <div className="flex items-center gap-3 px-4 py-3">
                          {attachmentCategory === 'image' && attachmentPreviewUrl ? (
                            <img src={attachmentPreviewUrl} alt="Preview" className="h-12 w-12 rounded-xl object-cover shrink-0 ring-2 ring-violet-500/30 shadow-sm" />
                          ) : (
                            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-violet-500/10 shrink-0 ring-1 ring-violet-500/20">
                  ...
                  <div className="relative">
                    ...
                    >
                        ...
                        <motion.button
                          ...
                          className={cn(
                            ...
                            attachmentFile
                              ...
                              : 'border-border/60 text-muted-foreground hover:border-violet-500/40 hover:text-violet-400 hover:bg-violet-500/5',
                          )}
                        >
                      ...
                      <div className="relative flex-1">
                        <input
                          ref={inputRef}
                          className="w-full rounded-2xl border border-border/60 bg-muted/50 px-4 py-2.5 pr-10 text-sm text-foreground placeholder:text-muted-foreground/50 focus:border-violet-500/50 focus:bg-muted/80 focus:outline-none focus:ring-2 focus:ring-violet-500/10 transition-all"
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            emojiOpen ? 'text-violet-400' : 'text-muted-foreground/40 hover:text-muted-foreground',
                          )}
                          title="Emojis"
                        >
